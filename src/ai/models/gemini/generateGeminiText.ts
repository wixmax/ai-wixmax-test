...
export const generateGeminiText = async (text: string, options: any = {}) => {
  console.log('Running handler with prompts:', options.editorSchema)
  const streamResult = await streamObject({
    maxTokens: options.maxTokens || 5000,
    model: google(options.model),
    onFinish: (result) => {
      console.log('Finished generating text with Gemini:', { options, rawResponse: result.rawResponse, result })
    },
    prompt: text,
